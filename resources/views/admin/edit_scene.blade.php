    ...
    <div class="card">
        <div class="card-body">
            @if (session()->has('errors'))
                <p>{{ session('errors') }}</p>
            @endif
            <form action="{{ route('dashboard_scene.update', $scene->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('put')
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control" id="hfov" name="hfov" value="{{ $scene->hfov }}"
                        required>
                </div>
                <div class="mb-3">
                    <label for="pitch" class="form-label">Pitch</label>
                    <input type="number" class="form-control" id="pitch" name="pitch" value="{{ $scene->pitch }}"
                        required>
                </div>
                <div class="mb-3">
                    <label for="yaw" class="form-label">Yaw</label>
                    <input type="number" class="form-control" id="yaw" name="yaw" value="{{ $scene->yaw }}"
                        required>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input class="form-control" id="panorama_img" name="img" type="file" onchange="previewImg()" />
                </div>
                <div class="mb-3">
